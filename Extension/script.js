...
document.querySelectorAll('input[name="data-source"]').forEach((radio) => {
  radio.addEventListener("change", (event) => {
    ...
    document.getElementById("all-datasets").parentElement.style.display =
      selectedSource === "upload" ? "block" : "none";
    document.getElementById("get-real-data-section").style.display =
      selectedSource === "real-data" ? "block" : "none";
  });
});
...
document.getElementById("generate-ai-prompt").addEventListener("click", async () => {
  ...
  if (selectedModel === "real-data") {
    ...
    const fullPrompt = `
     ...
     generate only from the given below Selected Text"
      ${Content}"
    `;
...
const GetSummarizationModel = async () => {
    
  const textToSummarize = document.getElementById("summarization-input").value;
  const SContent = document.getElementById("output").value;
  
fullsumm = `
    ${textToSummarize}
    for the below Selected Text
    "
    ${SContent}
    "
  `;
...
const GetWriteModel = async () => {
  ...
  const WContent = document.getElementById("output").value;
    sharedContext = `"
    ${WContent}"
  `;
